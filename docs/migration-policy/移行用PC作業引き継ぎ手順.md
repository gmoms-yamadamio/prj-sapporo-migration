...
| フォルダ | Git管理 | リモート | 備考 |
| --- | --- | --- | --- |
| `data-migration-etl` | ✅ 管理下に移行済み（本対応で`git init`実施） | **未設定**（下記§3参照） | 本ドキュメント作成と同時にリポジトリ化 |
...
## 3. `data-migration-etl` のリポジトリ設定（要対応）

`git init` と初回コミットは実施済み。**リモートリポジトリのURLが未確定**のため、以下は担当者側で実施すること。

```bash
cd data-migration-etl
git remote add origin <既存プライベートリポジトリのURL>
git push -u origin master
```

移行用PC側では通常の `git clone <URL>` で取得できる。
...
1. **GitHub認証情報の設定**
   ...
   - `data-migration-etl` 用の既存プライベートリポジトリ（§3のURL確定後）
...
| スクリプト・ドキュメント（`data-migration-etl`） | ○（本対応後） | Git経由でclone/pull |
...
- [ ] `data-migration-etl` 用の既存プライベートリポジトリURLを確認する
- [ ] `git remote add origin <URL>` → `git push -u origin master` を実行する
