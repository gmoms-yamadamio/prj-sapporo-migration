...
from lib.product_lookup import load_products, sku_for_product_id
...
def main() -> None:
    ...
    for order_id, order_lines in sorted(by_order.items()):
        for idx, line in enumerate(order_lines):
            ...
            if not sku:
                unmatched.append({"orderId": order_id, "productId": line["productId"]})
                continue
            output_rows.append({
                "注文番号": order_id,
                "注文明細番号": str(idx),
    ...
    checklist.record(REPORTS, "3.8", len(output_rows), source="generate_order_detail.py")
    print(f"order_detail.csv: {len(output_rows)} rows")
